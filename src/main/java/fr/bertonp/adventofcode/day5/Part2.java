    ...
    private static void mainCode(List<String> lines) throws Exception {
        ...
        for (String line : lines) {
            ...
            if (line.startsWith("seeds:")) {
                ...
                List<Long> seedsLongs = Arrays
                        .stream(seedsNumbers)
                        .filter(s -> !s.isBlank())
                        .map(Long::parseLong)
                        .toList();

                for (int i = 0; i < seedsLongs.size(); i += 2) {
                    long currentSeed = seedsLongs.get(i);
                    for (long j = currentSeed; j < currentSeed + seedsLongs.get(i + 1); j++) {
                        seeds.put(j, -1L);
                    }
                }
            } else {
